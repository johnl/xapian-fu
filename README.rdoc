...
XapianFu supports Xapian's `MatchAll` and `Match Nothing` queries. These are constants located in the `Xapian::Query` module

  db.search(Xapian::Query::MatchAll)
  db.search(Xapian::Query::MatchNothing)
